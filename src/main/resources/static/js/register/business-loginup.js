document.addEventListener('DOMContentLoaded', () => {
    ...
    const businessnumberMsg = document.getElementById('business-msg');
    ...
    function validateForm() {
        ...
        if (businessnumber.value.trim() === '') {
            businessnumberMsg.textContent = '* 사업자 번호는 필수 입력 항목입니다.';
            isValid = false;
        }else if (!/^\d{13}$/.test(businessnumber.value)) {
            businessnumberMsg.textContent = '사업자 번호는 13자리 숫자만 입력 가능합니다.';
            isValid = false;
        } else {
            businessnumberMsg.textContent = '';
        }
